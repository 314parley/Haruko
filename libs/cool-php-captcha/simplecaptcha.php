	...
	protected function WriteText($text, $fontcfg = array()) {
...

		{
			...
			for ($i=0; $i<$length; $i++) {
				...
				$fm = $this->im->queryFontMetrics($draw, $letter);

				$this->im->annotateImage($draw, $x, $y, $degree, $letter);

				$x += $fm['boundingBox']['x2'] + ($fontcfg['spacing']*$this->scale);
