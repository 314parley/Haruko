...
if (!empty($_POST['mode'])) {
    ...
    switch ($mode) {
        ...
        case "regist":
            ...
            if (($mod == 0) && ($mitsuba->common->isWhitelisted($mitsuba->common->getIP()) < 1)) {
            ...
            if (($mitsuba->common->isWhitelisted($mitsuba->common->getIP()) != 2) && (($mod == 0) || (!$mitsuba->admin->checkPermission("post.ignorespamlimits")))) {
            ...
            if (($bdata['type'] == "linkboard")) {
                ...
                if (!empty($_FILES['upfile']['tmp_name'])) {
                    ...
                    if (move_uploaded_file($_FILES['upfile']['tmp_name'], $target_path)) {
                        ...
                        if ($nfo['image'] == 1 || $nfo['extension'] == "webm") {
            ...
            if ((!empty($_POST['name'])) && (($bdata['noname'] == 0) || (($mod >= 1) && ($mitsuba->admin->checkPermission("post.ignorenoname"))))) {
            ...
            if (!empty($_POST['name'])) {
                ...
                setcookie("mitsuba_name", $_POST['name'], time() + 86400 * 256);

            } else {

                setcookie("mitsuba_name", "", time() + 86400 * 256);

            }

            if ((!empty($_POST['email'])) && (strtolower($_POST['email']) != "sage")) {

                setcookie("mitsuba_email", $_POST['email'], time() + 86400 * 256);

            } else {

                setcookie("mitsuba_email", "", time() + 86400 * 256);

            }

            if (!empty($_POST['fake_id'])) {

                setcookie("mitsuba_fakeid", $_POST['fake_id'], time() + 86400 * 256);

            } else {

                setcookie("mitsuba_fakeid", "", time() + 86400 * 256);
        ...
        case "usrform":
            ...
            if (!empty($_POST['delete'])) {
                ...
                if ((isset($_POST['onlyimgdel']) && ($_POST['onlyimgdel'] == "on"))) {
                ...
                foreach ($_POST as $key => $value) {
                    ...
                    if ($value == "delete") {
                        ...
                        if ($done == - 16) {
                            ...
                            echo "<h1>" . $lang['img/board_no_exists'] . "</h1></body></html>";
