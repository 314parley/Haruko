...
class Posting {
    ...
    function addPost($board, $name, $email, $subject, $comment, $password, $filename, $orig_filename, $mimetype = "", $resto = null, $md5 = "", $t_w = 0, $t_h = 0, $spoiler = 0, $embed = 0, $raw = 0, $sticky = 0, $locked = 0, $nolimit = 0, $nofile = 0, $fake_id = "", $cc_text = "", $cc_style = "", $cc_icon = "", $redirect = 0, $custom_fields = array()) {
        ...
        $pdata = array("board" => & $board, "name" => & $name, "trip" => & $trip, "strip" => & $strip, "email" => & $email, "subject" => & $subject, "comment" => & $comment, "password" => & $password, "filename" => & $filename, "orig_filename" => & $orig_filename, "mimetype" => & $mimetype, "resto" => & $resto, "md5" => & $md5, "t_w" => & $t_w, "t_h" => & $t_h, "spoiler" => & $spoiler, "embed" => & $embed, "raw" => & $raw, "sticky" => & $sticky, "locked" => & $locked, "nolimit" => & $nolimit, "nofile" => & $nofile, "fake_id" => & $fake_id, "cc_text" => & $cc_text, "cc_style" => & $cc_style, "cc_icon" => & $cc_icon, "custom_fields" => & $custom_fields);
        ...
        if (!empty($fake_id)) {
            ...
            if ($bdata['ids'] == 1) {
                if ($resto != 0) {
                    $poster_id = $this->mitsuba->common->mkid($_SERVER['HTTP_X_REAL_IP'], $resto, $board);
                }
            }
        }
        $isize = "";
        ...
        if (is_numeric($id)) {
            ...
            if ($result->num_rows == 1) {
                ...
                if ($result->num_rows == 0) {
                    ...
                    $this->conn->query("INSERT INTO reports (reporter_ip, reported_post, reason, created, board) VALUES ('" . $_SERVER['HTTP_X_REAL_IP'] . "', " . $id . ", '" . $reason . "', " . time() . ", '" . $board . "')");
                } else {
                    return 1;
                }
            } else {
