    ...
    function thumbnail($thumbnail_location, $max_w = 125, $max_h = 125, $overlay = "webm") {
        ...
        switch ($ext) {
            ...
            case 'webm':
                ...
                $this->exec_string = dirname(__FILE__) . '/ffmpeg -i ' . $this->input_file . ' -vcodec ' . $this->codec . ' -an' . ' -t ' . $this->max_time . " -vf scale=\"'if(gte(iw,ih),$max_w,-1)':'if(gte(iw,ih),-1,$max_h)'\",drawtext=\"fontfile=/usr/share/fonts/truetype/ttf-dejavu/DejaVuSans.ttf:  text='$overlay': fontcolor=white: x=2: y=(h-text_h)\" " . ' -y ' . $thumbnail_location . ' </dev/null 2>&1';

            break;

            case 'gif':

                $this->exec_string = dirname(__FILE__) . '/ffmpeg -i ' . $this->input_file . ' -t ' . $this->max_time . ' -r 10 ' . " -vf scale=\"'if(gte(iw,ih),$max_w,-1)':'if(gte(iw,ih),-1,$max_h)'\",drawtext=\"fontfile=/usr/share/fonts/truetype/ttf-dejavu/DejaVuSans.ttf:  text='$overlay': fontcolor=white: x=2: y=(h-text_h)\" " . ' -y ' . $thumbnail_location . ' </dev/null 2>&1';
