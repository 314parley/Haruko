...
class Board {
        ...
        while ($row = $spam->fetch_assoc()) {
            if ($row['boards'] != "%") {
                    ...
                    if ($row['regex'] == 1) {
                        try {
                            if (preg_match($row['search'], $comment)) {
                                $this->mitsuba->common->addSystemBan($_SERVER['HTTP_X_REAL_IP'], $row['reason'], htmlspecialchars($_POST['com']), $row['expires'], $row['boards']);
                ...
                if ($row['regex'] == 1) {
                    try {
                        if (preg_match($row['search'], $comment)) {
                            $this->mitsuba->common->addSystemBan($_SERVER['HTTP_X_REAL_IP'], $row['reason'], htmlspecialchars($_POST['com']), $row['expires'], "%");
                            echo '<meta http-equiv="refresh" content="2;URL=' . "'./banned.php'" . '">';
                            die();
                        }
                    }
    ...
    function checkThreadDate($bdata, $return_url) {
        global $lang;
        $lastdate = $this->conn->query("SELECT date FROM posts WHERE ip='" . $_SERVER['HTTP_X_REAL_IP'] . "' AND resto=0 AND board='" . $bdata['short'] . "' ORDER BY date DESC LIMIT 0, 1");
    ...
    function checkPostDate($bdata, $return_url) {
        global $lang;
        $lastdate = $this->conn->query("SELECT date FROM posts WHERE ip='" . $_SERVER['HTTP_X_REAL_IP'] . "' AND board='" . $bdata['short'] . "' ORDER BY date DESC LIMIT 0, 1");
        if ($lastdate->num_rows == 1) {
            $pdate = $lastdate->fetch_assoc();
            $pdate = $pdate['date'];
